...
The board routes signal, power, and ground traces such that each IC slot is electrically independent, so a failed device does not affect adjacent socket measurements.
